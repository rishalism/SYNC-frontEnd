import { addMemberIntoProject } from '@/api/projectsApi'
import { AcceptProjectInvitation } from '@/api/teamMemberApi'
...
function AcceptInvitation() {
    ...
    const handleInvitation = async () => {
        if (!TeamMemberInfo) {
            ...
            const response = await AcceptProjectInvitation({ token, projectId })
            if (response) {
                const data = await addMemberIntoProject({ memberId: TeamMemberInfo.id, projectId })
                if (data) {
                    navigate('/overview')
                    toast.success(data.data, { position: 'top-center' })
                }
            }
        }
    }
        ...
        <>
            <div className="w-full h-screen flex justify-center items-center">
                <div className="flex flex-col items-center gap-8">
                    ...
                    <Button onClick={handleInvitation} className='w-full capitalize mt-8 ' variant={'default'}>accept invite !</Button>
                </div>
            </div>
